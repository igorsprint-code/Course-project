...
## Grafana: 111.88.243.2:3000
## Kibana: 158.160.60.109:5601
...
Файл [sec_groups.tf](/terraform/sec_groups.tf) содержит конфигурацию групп безопасности:

Группы безопасности для ВМ. Группы "bastion", "grafana", "kibana" ограничивают доступ к соответствующим ВМ, доступ возможен только по портам, которые прослушивают сервисы ( доступ к "bastion" только по порту 22, к "kibana" только по порту 5601, к "grafana" только по порту 3000, для всех ВМ разрешено подключение по ssh)

Файл [balancer.tf](/terraform/balancer.tf) содержит описание Target group, с включеннымими в неё web-серверами, Backend group , HTTP router и Application load balancer. Настроен healthcheck.

В файле [vms.tf](/terraform/vms.tf) определены ВМ: "bastion", "web_1", "web_2", "prometheus_vm", "grafana_vm", "elasticsearch_vm", "kibana_vm". Описаны подсети в которых они будут работать, железо, ОС,  назначены группы безопасности. Файл также содержит конструкцию, позволяющую после создания инфрастурктуры записать в файл hosts.ini ip созданных ВМ, позже этот файл будет выполнять роль inventory-файла для ansible.

В файле [snapshot.tf](/terraform/snapshot.tf) описан порядок создания  snapshot дисков всех ВМ.

Cloud_id и folder_id задаются в файле [providers.tf](/terraform/providers.tf) через переменные var.cloud_id и var.folder_id. Значения перменных содержатся в файле [variables.tf](/terraform/variables.tf).

SSH-ключ задается в файле [cloud-init.yml](/terraform/cloud-init.yml), через переменную ssh_public_key. Значение переменной задается в файле terraform.tfvars.

Актуальные IP можно поулчить файла [output.tf](/terraform/output.tf) через команду terrraform output.
...
В браузере по адресу балансировщика  видим стартовую страницу с одного и второго сервера. Балансировщик работает!
...
Kibana доступна  и получает данные от elasticsearch:
...
Инфраструктура для сайта работает, включает в себя мониторинг, сбор логов и резервное копирование основных данных. 
