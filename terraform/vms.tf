...
resource "local_file" "inventory" {
  ...
  [grafana_server]
  ${yandex_compute_instance.grafana_vm.network_interface.0.ip_address}
  [grafana_server:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -p 22 -W %h:%p -q user@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'


  [elasticsearch_server]
  ${yandex_compute_instance.elasticsearch_vm.network_interface.0.ip_address}
  [elasticsearch_server:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -p 22 -W %h:%p -q user@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'


  [kibana_server]
  ${yandex_compute_instance.kibana_vm.network_interface.0.ip_address}
  [kibana_server:vars]
  ansible_ssh_common_args='-o ProxyCommand="ssh -p 22 -W %h:%p -q user@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'
